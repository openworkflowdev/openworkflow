...
Workflows are the core building block of OpenWorkflow. They are durable
functions that can survive process crashes, server restarts, and code deploys.
When a workflow is interrupted, it resumes from its last completed step.

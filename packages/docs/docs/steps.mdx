...
and its result is memoized. If a workflow restarts, completed steps return their
cached results immediately instead of re-running.
...
<Warning>
  ...
  Completed steps won't be found in the history, causing them to re-execute.
</Warning>

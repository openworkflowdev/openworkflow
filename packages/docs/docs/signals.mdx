...
// `decision` is typed as { approved: boolean; reviewedBy: string } | null
```
...
const workflowA = defineWorkflow({ name: "listener-a" }, async ({ step }) => {
  const data = await step.waitForSignal({ signal: "config-updated" });
  // handle update
});
  ...
  async ({ input, step }) => {
    ...
    if (notification) {
      await step.run({ name: "process-data" }, async () => {
        await processRecords(input.batchId, notification.recordCount);
      });
    }
  },
);

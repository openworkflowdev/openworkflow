...
openworkflow_workflow_runs{status="sleeping"} 1
...
| Type   | Gauge                                                               |
| Labels | `status`                                                            |
| Values | `pending`, `running`, `sleeping`, `completed`, `failed`, `canceled` |

Current count of workflow runs in each status. One query is executed per
scrape — there is no caching.

- **Active statuses** (`pending`, `running`, `sleeping`) — Is work piling up?
  Are runs stuck?

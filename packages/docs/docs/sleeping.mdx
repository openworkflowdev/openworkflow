...
Workflows can pause for any duration using `step.sleep()`. This is a durable
pause that survives crashes and restarts. Unlike `setTimeout` or `await
sleep()`, a workflow sleep releases the worker slot so you can pause thousands
of workflows without tying up compute.

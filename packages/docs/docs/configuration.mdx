...
Automatic cleanup policy for workflow runs in terminal states (`completed`,
`failed`, `canceled`):

```ts
retention: {
  enabled: true,
  period: "30d", // delete terminal runs older than 30 days
},
```

When enabled, running workers periodically remove terminal runs older than
`period`.

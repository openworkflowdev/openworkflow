---
title: Signals
description: Send data between workflows at runtime without polling and wait for approvals or webhook callbacks with a timeout
---
...
A single `sendSignal` call delivers to every workflow currently waiting on that
signal name:

---
title: Retries
description: Automatic retry behavior for failed steps and workflows, with exponential backoff and reuse of completed step results
---
...
Use `rerunWorkflowRun` to create a new run from a failed, completed, or canceled
run. Optionally pass a recorded step name to reuse earlier results:

```ts
const newRun = await ow.rerunWorkflowRun(originalRunId, {
  fromStep: "send-email", // <-- optional
});
```

The new run uses the original input and workflow version. It is ready to run
immediately, with fresh retry counts and no deadline, idempotency key, or parent
run. The source run stays unchanged.

With `fromStep`, OpenWorkflow copies successful steps from before the selected
step. The new run reuses those results.
...
Errors thrown outside of `step.run(...)` are workflow-level failures. They are
not retried by default; the workflow is marked as `failed`.

---
title: CLI Reference
description: Command-line interface for OpenWorkflow, with commands to set up a project, start workers, and open the dashboard
---

The OpenWorkflow CLI
[(`@openworkflow/cli`)](https://www.npmjs.com/package/@openworkflow/cli) is the
primary way to set up your project, run workers, and launch the dashboard. The
CLI reads your `openworkflow.config.ts` by default, or you can override the path
with `--config`.
...
```bash
npx --yes @openworkflow/cli init --backend postgres --yes --skip-install
```
...
The CLI sends command names, backend types, CLI/Node.js versions, OS,
architecture, and a random installation ID to PostHog by default. It does not
send project contents, paths, or error messages.

Disable telemetry with `--no-telemetry` or `DO_NOT_TRACK=1` in your shell.
Any nonempty `CI` or `OPENWORKFLOW_TELEMETRY_DISABLED` also disables it.

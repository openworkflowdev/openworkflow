---
title: Quick Start
description: Get up and running with OpenWorkflow in 5 minutes, from installing the CLI to running and inspecting a workflow
---
...
For unattended setup from a script or agent, pass both `--backend` and `--yes`:

...
retries with backoff, optional `deadlineAt`) for TypeScript teams that want
fewer moving parts.

### Tradeoff: happy-path overhead vs resume-path replay CPU

OpenWorkflow uses fewer orchestration hops on the common path and accepts
higher replay CPU when runs resume after retries, crashes, sleeps, or deploys.

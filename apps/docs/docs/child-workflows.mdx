...
description: Run workflows from other workflows and wait for their results without holding a worker slot while the child runs
---
...
<Warning>
  When the timeout is reached, the parent step fails, but the child workflow
  keeps running independently. The child is not automatically canceled.
</Warning>

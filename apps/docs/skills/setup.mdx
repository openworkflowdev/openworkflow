---
title: Setup
description: Instructions for coding agents to install OpenWorkflow and verify a local workflow run using SQLite or PostgreSQL
---
...
command is interactive. In the npm command, the first `--yes` also skips the
package-install prompt. Setup installs dependencies and creates a config,

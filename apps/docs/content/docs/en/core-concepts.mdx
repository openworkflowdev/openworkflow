...
<Callout type="error" title="Dangerous Without Steps">
...
<Callout type="success" title="Safe With Steps">
...
| `sleeping` | Workflow is paused until a future time (`step.sleep`) |
| `succeeded` | Workflow completed successfully |
| `failed` | Workflow failed and all retries exhausted |
| `canceled` | Workflow was explicitly canceled and will not continue |

### Lifecycle Flow

```
pending → running → sleeping → running → succeeded
                  ↘ failed
                  ↘ canceled
```
...
- **Heartbeat**: `availableAt = NOW() + leaseDuration`
 - **Sleep wake-up**: `availableAt = resumeTime` when a workflow calls `step.sleep`
...
<Files>
  <Folder name="src" defaultOpen>
    <Folder name="workflows" defaultOpen>
      <File name="send-welcome-email.ts" />
      <File name="process-order.ts" />
      <File name="sync-user-data.ts" />
      <File name="index.ts" />
    </Folder>
    <File name="worker.ts" />
    <File name="client.ts" />

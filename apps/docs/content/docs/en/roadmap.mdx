...
OpenWorkflow is currently in **v0.2** and under active development. The core features are stable and production-ready, but we're continuously adding new capabilities based on community feedback.
</Callout>

## Current Status

The current release includes core functionality for building durable workflows:
...
- **Sleeping (Pausing) Workflows** - Durable timers via `step.sleep` that free worker slots
...
Core features for building production-ready durable workflows, plus:
- Sleep workflows with `step.sleep(name, duration)`

### v0.1.0

Initial release with core durable workflow features:
- PostgreSQL backend
...
- **v0.4.x** - CLI and Dashboard UI
- **v0.5.x** - Additional backends (Redis, SQLite)

...
function summarizeCustomSchema(value: unknown) {
  if (typeof value !== "object" || value === null) {
    throw new TypeError("Workflow input must be an object");
  }

  const { docUrl, num } = value as {
    docUrl?: unknown;
    num?: unknown;
  };

  if (typeof docUrl !== "string") {
    throw new TypeError("docUrl must be a string");
  }

  try {
    // ensure docUrl is a well-formed URL
    new URL(docUrl);
  } catch {
    throw new TypeError("docUrl must be a valid URL");
  }
 ...
 * from a URL. It uses a custom validator function to validate the input.
 */
const summarizeDoc = ow.defineWorkflow(
  { name: "summarize-doc-custom", schema: summarizeCustomSchema },
  async ({ input, step }) => {
